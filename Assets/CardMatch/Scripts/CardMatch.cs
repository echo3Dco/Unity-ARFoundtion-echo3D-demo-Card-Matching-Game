...
public class CardMatch : MonoBehaviour
{
    ...
    IEnumerator QueryDatabase(string serverURL)
    {
        ...
        else
        {
            ...
            // Initialize the CardLayoutManager
            TOTAL_CARDS = dbObject.getEntries().Count * MULTIPLIER;
            cardLayoutManager = new CardLayoutManager(TOTAL_CARDS, CARD_SPACING);
        }
    ...
    public void ParseDatabase(string json)
    {
        ...
        if (!json.ToLower().Contains("not found"))
        {
            ...
            Debug.Log("CardLayoutManager initialized.");
    ...
    IEnumerator DownloadAssets(string serverURL)
    {
        ...
        {
            DownloadEntryAssets(entry, serverURL, MULTIPLIER);
    ...
    public void DownloadEntryAssets(Entry entry, string serverURL, int multiplier)
    {
        ...
        for (int m = 0; m < multiplier; m++)
        {
            if (true)
            {
                ...
                else if (hologramType.Equals(Hologram.hologramType.VIDEO_HOLOGRAM))
                {
                    ...
                    // Attach a CardItem Component
                    var cardItem = videoPlane.AddComponent<CardItem>();
                    cardItem.ID = entry.getId();
                    cardItem.cardLayoutManager = cardLayoutManager;
    ...
    IEnumerator DownloadandInitiateImage(Entry entry, string imageURL, GameObject imagePlane)
    {
        ...
        // Attach a CardItem Component
        var cardItem = imagePlane.AddComponent<CardItem>();
        cardItem.ID = entry.getId();
        cardItem.cardLayoutManager = cardLayoutManager;
    }
    ...
    IEnumerator InstantiateModel(Entry entry, List<string> filenames, ImportOptions importOptions)
    {
        ...
        if (extension == ".glb")
        {
            ...
            var cardItem = result.AddComponent<CardItem>();
            cardItem.ID = entry.getId();
            cardItem.cardLayoutManager = cardLayoutManager;
    ...
    IEnumerator WebsocketClient()
    {
        ...
        WClient.On(WClient.EventType.DELETE_ENTRY.ToString(), (string message) =>
        {
            ...
            foreach (CardItem cb in FindObjectsOfType<CardItem>())
            {
